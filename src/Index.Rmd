...
## Checks Charlotte
```{r mixeddictests8, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
data$NEUROinv <- factor(data$NEURO, levels(data$NEURO)[c(2,1)])
fm8 <- glmmTMB(mmse ~ logtime + NEUROinv + NEUROinv * logtime + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity")) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm8 <- summary(fm8)
sfm8$coefficients$cond %>% kable(caption = "Model with random intercept and slope.",
        booktabs = T
)
```


```{r mixeddictests9, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
data$logtimesquared <- data$logtime * data$logtime
fm9 <- glmmTMB(mmse ~ logtime +  logtimesquared + NEUROinv + NEUROinv * logtime + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm9 <- summary(fm9)
sfm9$coefficients$cond %>% kable(caption = "Model with random intercept and slope, including squared log(time).",
        booktabs = T
)
```

```{r mixeddictests10, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
fm10 <- glmmTMB(mmse ~ -1 + logtime +  logtimesquared + NEUROinv + NEUROinv * logtime + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm10 <- summary(fm10)
sfm10$coefficients$cond %>% kable(caption = "Model with random intercept and slope (noint), including squared log(time).",
        booktabs = T
)
```


```{r mixeddictests10b, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
fm10b <- glmmTMB(mmse ~ -1 + logtime +  logtimesquared + NEUROinv + NEUROinv * logtime + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm10b <- summary(fm10b)
sfm10b$coefficients$cond %>% kable(caption = "Model with random intercept and slope (noint), including squared log(time).",
        booktabs = T
)
```

```{r mixeddictests11, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
data$housinginv <- factor(data$housing,levels = levels(data$housing)[c(3,1,2)])
data$age65 <- data$age - 65
fm11 <- glmmTMB(mmse ~ -1 + logtimesquared + NEUROinv + NEUROinv:logtime + logtime:age65 + age65 + housinginv + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm11 <- summary(fm11)
sfm11$coefficients$cond %>% kable(caption = "Model with random intercept and slope, including squared log(time), age, housing.",
        booktabs = T
)
```


```{r mixeddictests12, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
fm12 <- glmmTMB(mmse ~ logtime + logtimesquared + NEUROinv + NEUROinv * logtime + logtime*age65 + age65 + housing + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm12 <- summary(fm12)
sfm12$coefficients$cond %>% kable(caption = "Model with random intercept and slope, including squared log(time), age, housing.",
        booktabs = T
)
```

```{r mixeddictests13, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
fm13 <- glmmTMB(mmse ~ -1 + logtimesquared + NEUROinv + NEUROinv:logtime + logtime:age65 + age65 + housinginv + housinginv:logtime + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm13 <- summary(fm13)
sfm13$coefficients$cond %>% kable(caption = "Model with random intercept and slope, including squared log(time), age, housing. (p22-23)",
        booktabs = T
)
```

```{r mixeddictests14, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
fm14 <- glmmTMB(mmse ~ 1+ logtime + logtimesquared + NEUROinv + NEUROinv:logtime + logtime:age65 + age65 + housinginv + housinginv:logtime + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm14 <- summary(fm14)
sfm14$coefficients$cond %>% kable(caption = "Model with random intercept and slope, including squared log(time), age, housing. (p26)",
        booktabs = T
)
```

```{r mixeddictests15, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
fm15 <- glmmTMB(mmse ~ 1 + logtime + NEUROinv + + age65 + housinginv + (1 + logtime|id), 
             data = data, family = gaussian(link = "identity"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm15 <- summary(fm15)
sfm15$coefficients$cond %>% kable(caption = "Model with random intercept and slope, including  age, housing. (p28)",
        booktabs = T
)
datafilter <- data %>% filter(!is.na(housing))
```
Een model zoals pagina 31 kan ik niet fitten er zijn geen patienten met een normaal mmse bij de neuro-psychiatrische patienten in nursing homes.

```{r mixeddictests16, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE}
fm16 <- glmmTMB(bmmse ~ -1 + logtimesquared + NEUROinv + logtime:NEUROinv + age65 + logtime:age + housinginv + (1 + logtime|id), 
             data = datafilter, family = binomial(link = "logit"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm16 <- summary(fm16)
sfm16$coefficients$cond %>% kable(caption = "full binomial model, logit link. FIltered data (no missing housing). (p28)",
        booktabs = T
)
datafilter %>% group_by(bmmse, NEURO, housing) %>% summarize(n=n())
datafilter %>% group_by(bmmse, NEURO) %>% summarize(n=n())
```


```{r mixeddictests17, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE}
fm17 <- glmmTMB(bmmse ~ 1 + logtime + NEUROinv + logtime*NEUROinv + age + housing + (1 + logtime|id), 
             data = datafilter, family = binomial(link = "logit"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm17 <- summary(fm17)
sfm17$coefficients$cond %>% kable(caption = "binomial model, logit link. Filtered data (no missing housing). (p36)",
        booktabs = T
)
```

```{r mixeddictests18, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE}
fm18 <- glmmTMB(bmmse ~ 1 + NEUROinv + age + housing + (1 |id), 
             data = datafilter, family = binomial(link = "logit"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm18 <- summary(fm18)
sfm18$coefficients$cond %>% kable(caption = "binomial model, logit link. Filtered data (no missing housing). (p36)",
        booktabs = T
)
```


```{r mixeddictests19, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE}
fm19 <- glmmTMB(bmmse ~ 1 + logtime + NEUROinv + logtime*NEUROinv  + (1 + logtime|id), 
             data = datafilter, family = binomial(link = "logit"), REML = TRUE) #In checkConv(attr(opt, "derivs"), opt$par, ctrl = control$checkConv,  :
  #Model failed to converge with max|grad| = 0.428569 (tol = 0.002, component 1)
sfm19 <- summary(fm19)
sfm19$coefficients$cond %>% kable(caption = "full binomial model, logit link. FIltered data (no missing housing). ",
        booktabs = T
)
```
