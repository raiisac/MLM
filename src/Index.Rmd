...
Looking at the completeness of the data, It can be observed that `r n_distinct(data[is.na(data$housing),"id"])` persons' housing situation is unknown. Additionally, there was some dropout over time as show in Table \@ref(tab: EDA_dropout).

```{r EDA_dropout, echo = F}
data %>% group_by(time) %>%
  ...
  dplyr::select(time, nb, return, dropout, avgage, avgneuro, avgalone, avgfp, 
                avgnurse, avgna) %>%
  kable(col.names = c("Time", "Number", "Return", "Dropout", "Mean age", 
...
```{r EDA_MMSE_time, echo = FALSE, message= FALSE, warning = FALSE, fig.cap = "MMSE over time for all patient profiles, including smoothed curves."}
ggplot(data) + 
