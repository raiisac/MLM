...
```{r simplemixedrandomdotplot, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "This shows both random effects in the model for each subject (y-axis). It is clear that there is very little variation in the slopes."}
...
reff$id %>%
  ...
  ggplot() + 
  geom_point(aes(y = `(Intercept)`, x = logtime, color = NEURO)) +
  theme_bw() + ylab("Intercept") + xlab("logtime")
```
...
```{r simplemixedrandomdotplotneuro, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "This shows both random effects in the model for each subject (y-axis). It is clear that there is very little variation in the slopes."}
...
reff$id %>%
  ...
  left_join(distinct(data[,c('id','NEURO')])) %>%
  ggplot() +
  geom_point(aes(y = `(Intercept)`, x = `NEUROneuro-psychiatric`, color = NEURO)) +
  theme_bw() + ylab("Intercept") + xlab("NEURO")
```
...
fm3fulltest <- lmerTest::lmer(mmse ~ logtime + NEURO + NEURO * logtime + age65 * logtime + housinginv + (1 + logtime|id), data = data) 
...
fm3fulltest <- lmerTest::lmer(mmse ~ logtime + NEURO +  age65 * logtime + housinginv + (1 + logtime|id), data = data) 
...
fm3fulltest <- lmerTest::lmer(mmse ~ logtime + NEURO +  age65 + housinginv + (1 + logtime|id), data = data) 
AIC(fm3full) - AIC(fm3fulltest)#more positive
#summary(fm3fulltest)$coefficients #least significant is the interaction with time and age
fm3reduced <- lmerTest::lmer(mmse ~ logtime + NEURO +  age65 + housinginv + (1 + logtime|id), data = data) 
library("cAIC4")
cAIC(fm3full) 
cAIC(fm3reduced)#mo
...
```{r mixedcovfittedreduced, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Fitted values plotted against log(time) and time. Dashed, thicker lines are the predicted trends based on the fixed effects only, for an average-aged patient in each group.", fig.height = 10, fig.width = 7}
...
fm6 <- glmmTMB(bmmse ~ 1 + logtime + NEURO + logtime*NEURO + age65*logtime + logtime*housinginv + (1 + logtime|id), 
...
```{r mixeddictests2, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE}
data$bmmse <- factor(as.factor(ifelse(data$mmse <= 24,"abnormal","normal")), levels = c("abnormal","normal"))
fm7 <- glmmTMB(bmmse ~ 1 + logtime + NEURO + logtime*NEURO + age65*logtime + logtime*housinginv + (1 + NEURO|id),
...
sfm7$coefficients$cond %>% kable(caption = "Fixed effects for a model with random intercept and slope, including age and housing, dichotomized dependent variable (split at 25).",
                            booktabs = TRUE, digits = 3)
```
# Conclusion
